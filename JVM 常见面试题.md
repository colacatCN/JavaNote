...
2. 准备：为类中的静态字段分配内存并设置默认的初始值。需要注意的是，被 final 关键字修饰的静态字段在编译期就已经被初始化。

3. 解析：将方法区的运行常量池中的符号引用替换为直接引用（ 真实的内存地址 ）。

### 初始化

初始化是类加载的最后一步，是执行 clinit() 类构造方法的过程，其作用是去初始化**静态字段**和**静态代码块**。若该类具有父类，JVM 会保证父类的 clinit() 方法先被执行，然后再执行子类的 clinit() 方法。
...
初始化成员变量，执行实例代码块，调用类的构造方法，并把堆内存中该对象的首地址赋值给虚拟机栈上的引用变量。
