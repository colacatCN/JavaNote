...
* 在 DirectCallAdapterFactory 这个工厂实现类中是直接调用 Call 对象的 execute() 发送请求，将服务端返回的结果反序列化后返回给业务层。

* 在 Java8CallAdapterFactory 这个工厂实现类中是将 Call 对象封装进了 CompletableFuture 中返回给业务层。
