...
在 MySQL 建立联合索引时会遵守最左前缀匹配原则，即联合索引中字段的顺序意味着索引首先会按照最左边的字段 col1 构建整棵 B+ 树，其次在 col1 确定的情况下再进行字段 col2 的排序，col2 是相对于 col1 有序的。实际上联合索引 index_col1_col2_col3 建立了一个单列（ col1 ）和两个（ col1，col2 ）、（ col1，col2，col3 ） 组合索引。
