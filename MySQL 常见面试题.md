...
以主键以外的列作为**二级主键**构建的 B+ 索引树，称之为非聚簇索引。
...
覆盖索引就是在一个联合索引中包含（ 或者说覆盖 ）了所有需要查询的字段。简而言之，当 SQL 语句中所查询的字段 `select` 和查询条件的字段 `where` 包含在一个联合索引中，这样就可以直接使用二级主键查询出的结果，而不再需要回表到一级主键所构建的 B+ 树中进行二次查询。
